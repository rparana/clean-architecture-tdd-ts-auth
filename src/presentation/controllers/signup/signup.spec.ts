...
const makeSut = (): SutTypes => {
  ...
  const sut = new SignUpController(emailValidatorStub, addAccountStub, validationtStub)
  return {
    sut,
    emailValidatorStub,
    addAccountStub,
...
describe('SignUp Controller', () => {
  test('Should return 400 if invalid password confirmation is provided', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('Should call EmailValidator with correct email', async () => {
    ...
    expect(isValidySpy).toHaveBeenCalledWith('valid_email@mail.com')
  })
